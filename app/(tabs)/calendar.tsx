...
import { Plus, X, ChevronLeft, ChevronRight, RefreshCw } from 'lucide-react-native';
...
export default function CalendarScreen() {
  ...
  const { user } = useAuth();
  const { addAppointment, getAppointmentsForDate, refreshAppointments, setUserTypeAndReload } = useAppointments();
  ...
  const handleAddPersonalTask = async () => {
    const { title, startTime, endTime, date, notes } = newPersonalTask;
    if (!title || !startTime || !endTime) {
      Alert.alert('Datos incompletos', 'Por favor ingresa un título, hora de inicio y hora de fin.');
      return;
    }
    ...
    const appointmentToAdd: Omit<Appointment, 'id' | 'confirmationPostpones'> = {
      ...
      service: title,
      type: 'blocked', // Time blocks are always of type 'blocked'
      status: 'confirmed',
      notes: notes,
    };
    ...
    try {
      ...
      setNewPersonalTask({ title: '', startTime: '', endTime: '', notes: '', date: selectedDate });
      Alert.alert('Éxito', 'Tu agenda personal ha sido actualizada.');
    } catch (error) {
      console.error('Error saving personal task:', error);
      Alert.alert('Error', 'No se pudo guardar el evento en tu agenda.');
    }
  };
  ...
  const handleOpenTimePicker = (type: 'start' | 'end') => {
    if (!type?.trim()) return;
    setPickingFor(type);
    setModalState(prev => ({...prev, timePicker: true}));
  };

  const handleTimeSelect = (time: string) => {
    if (pickingFor === 'start') {
      setNewPersonalTask(prev => ({ ...prev, startTime: time, endTime: '' }));
    } else if (pickingFor === 'end') {
      setNewPersonalTask(prev => ({ ...prev, endTime: time }));
    }
    setModalState(prev => ({...prev, timePicker: false}));
    setPickingFor(null);
  };
        ...
        <View style={styles.selectedDateSection}>
          ...
          {selectedDateAppointments.map(app => {
            ...
            <TouchableOpacity key={app.id} style={[styles.appointmentDetailCard, { borderLeftColor: getEventColor(app.type) }]} onPress={() => { setSelectedReservation(app); setModalState(prev => ({ ...prev, reservationDetails: true })); }}>
              ...
              <Text style={styles.appointmentDetailClient}>{app.type.includes('personal') || app.type.includes('blocked') ? 'Evento Personal' : `Proveedor: ${app.clientName}`}</Text>
      ...
      <Modal visible={modalState.personalTask} transparent animationType="slide" onRequestClose={() => setModalState(prev => ({ ...prev, personalTask: false }))}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}><Text style={styles.modalTitle}>Bloquear Horario</Text><TouchableOpacity onPress={() => setModalState(prev => ({ ...prev, personalTask: false }))}><X size={24} color="#666" /></TouchableOpacity></View>
            <ScrollView>
              <View style={styles.inputGroup}><Text style={styles.inputLabel}>Título del Bloqueo *</Text><TextInput style={styles.textInput} value={newPersonalTask.title} onChangeText={text => setNewPersonalTask(prev => ({ ...prev, title: text }))} placeholder="Ej: Horario de Trabajo, Clases" /></View>
              
              <View style={styles.timeSelectionContainer}>
                <View style={styles.timeInputWrapper}>
                  <Text style={styles.inputLabel}>Hora de Inicio *</Text>
                  <TouchableOpacity style={styles.timeButton} onPress={() => handleOpenTimePicker('start')}>
                    <Text style={styles.timeButtonText}>{newPersonalTask.startTime || 'Seleccionar'}</Text>
                  </TouchableOpacity>
                </View>
                <View style={styles.timeInputWrapper}>
                  <Text style={styles.inputLabel}>Hora de Fin *</Text>
                  <TouchableOpacity style={styles.timeButton} onPress={() => handleOpenTimePicker('end')} disabled={!newPersonalTask.startTime}>
                    <Text style={styles.timeButtonText}>{newPersonalTask.endTime || 'Seleccionar'}</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={styles.saveButton} onPress={handleAddPersonalTask}><Text style={styles.saveButtonText}>Guardar Bloqueo</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
            ...
            <View style={styles.timePickerContainer}>
              ...
              <ScrollView>
                <View style={styles.timePickerGrid}>
                  {Array.from({ length: 14 }, (_, i) => i + 7).flatMap(hour => [0, 30].map(minute => { // Hours from 7am to 9pm
                    const timeString = `${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}`;
                    const isDisabled = pickingFor === 'end' && newPersonalTask.startTime >= timeString;
                    return (
                      <TouchableOpacity key={timeString} style={[styles.timePickerOption, isDisabled && styles.timePickerOptionDisabled]} onPress={() => handleTimeSelect(timeString)} disabled={isDisabled}>
                        <Text style={[styles.timePickerOptionText, isDisabled && styles.timePickerOptionTextDisabled]}>{timeString}</Text>
                      </TouchableOpacity>
                    );
                  }))}
                </View>
...
const styles = StyleSheet.create({
  ...
  loadingText: { flex: 1, textAlign: 'center', textAlignVertical: 'center', fontSize: 18 },
  ...
  calendarHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 16 },
  navButton: { padding: 8 },
  ...
  dayHeaderText: { fontSize: 12, fontWeight: '500', color: '#666', textTransform: 'uppercase' },
  calendarGrid: { flexDirection: 'row', flexWrap: 'wrap' },
  dayCell: { width: `${100/7}%`, height: 70, padding: 4, borderRightWidth: 0.5, borderBottomWidth: 0.5, borderColor: '#E5E5E5' },
  ...
  dayNumber: { fontSize: 14, fontWeight: '600', color: '#333', textAlign: 'center' },
  dayNumberOtherMonth: { color: '#999' },
  dayNumberToday: { color: '#D81B60', fontWeight: 'bold' },
  dayNumberSelected: { color: '#2196F3', fontWeight: 'bold' },
  eventsContainer: { flex: 1, gap: 2, paddingTop: 4 },
  eventIndicator: { height: 4, borderRadius: 2, marginHorizontal: 4 },
  ...
  appointmentDetailCard: { backgroundColor: 'white', borderRadius: 12, padding: 16, marginBottom: 12, borderLeftWidth: 5, elevation: 2 },
  ...
  personalAgendaSection: { backgroundColor: 'white', marginHorizontal: 16, marginVertical: 16, borderRadius: 12, padding: 20, elevation: 2, alignItems: 'center' },
  ...
  addPersonalAgendaButton: { backgroundColor: '#9C27B0', borderRadius: 12, paddingVertical: 16, paddingHorizontal: 24, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  ...
  timeSelectionContainer: { flexDirection: 'row', justifyContent: 'space-between', gap: 16, marginBottom: 16 },
  ...
  timePickerOption: { padding: 12, borderRadius: 8, margin: 4, backgroundColor: '#F5F5F5', minWidth: 80, alignItems: 'center' },
  timePickerOptionDisabled: { backgroundColor: '#E0E0E0' },
  timePickerOptionText: { fontSize: 16, color: '#333' },
  timePickerOptionTextDisabled: { color: '#9E9E9E' },
